...
Description: Set up the required namespace and permissions for Kubetrace.
...
$KUBECTL_ADMIN create clusterrolebinding "$CLUSTER_BINDING"   --clusterrole="$CLUSTER_ROLE"   --serviceaccount="$NAMESPACE:$SA_NAME"   --dry-run=client -o yaml | $KUBECTL_ADMIN apply -f -
...
```
When you run this script, Kubetrace will automatically provision the following components in your cluster:
    Namespace:
    ```sh
    apiVersion: v1
    kind: Namespace
